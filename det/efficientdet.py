...
import pickle
import os
from det.nets.efficientdet import Efficientdet
from keras import backend as K
from keras.layers import Input
from PIL import Image,ImageFont, ImageDraw
from det.utils.utils import BBoxUtility,letterbox_image,efficientdet_correct_boxes
from det.utils.anchors import get_anchors
...
class EfficientDet(object):
    _defaults = {
        "model_path": 'det/weight.h5',
        "classes_path": 'det/model_data/voc_classes.txt',
        "phi": 0,
        "confidence": 0.26,
    }
    ...
    def detect_image(self, image):
        ...
        font = ImageFont.truetype(font='det/model_data/simhei.ttf',size=np.floor(3e-2 * np.shape(image)[1] + 0.5).astype('int32'))

        thickness = (np.shape(image)[0] + np.shape(image)[1]) // self.model_image_size[0]

        if  len(top_label_indices) > 1:
            text = 'Found {} boxes for {} \n'.format(len(top_label_indices), 'image')
        else:
            text = 'Found {} box for {} \n'.format(len(top_label_indices), 'image')
        ...
        for i, c in enumerate(top_label_indices):
            ...
            score = top_conf[i]
            text += 'Box'+str(i+1)+':'+predicted_class +'   score='+str(round(score,3))+'\n'
